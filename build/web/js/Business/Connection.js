...
function Connection( host,
    ...
    function beginSession( conn ) {
        ...
        conn.sendMailNotifyRequest();
        var mtid = window.setInterval( mailThread, MT_REFRESH_DELAY, conn );
    }
  ...
  function onReceivedPacketFromProxy(req, conn ) {
        ...
        if (packetList.tagName == "iq") {
            ...
            if (conn.getProfile().getIsGoogle() == 1)
            {
                ...
                if( mailbox && mailbox.getAttribute("result-time") != "") {
                    ...
                    conn.getProfile().setLastTimeMailSeen( resultTime );
                    
                    var v = packetList.getElementsByTagName("mailbox").item(0).childNodes;
                    var i;
                    for (i = v.length -1; i >= 0; i--) {
                        var t = v[i];
                        var sender = t.getElementsByTagName("senders").item(0).getElementsByTagName("sender").item(0).getAttribute("address");
                        var subject = "";
                        var body = "";
                        try {
                          subject = t.getElementsByTagName("subject").item(0).firstChild.nodeValue;
                          body = t.getElementsByTagName("snippit").item(0).firstChild.nodeValue;
                        } catch( e ) {}
                        emails.push(new Email(sender, subject, body));
                    }
                    var newTID = v[ i + 1 ].getAttribute( "tid" );
                    conn.getProfile().setLastTID( newTID );
                    conn.onMailReceived();
                }
    ...
    this.sendMailNotifyRequest = function() {
        ...
        var tid = this.getProfile().getLastTID();
        ...
        var qs = "a=getmail&time=" + time + "&tid=" + tid;
        sendSimplePacket( qs );         
    };
